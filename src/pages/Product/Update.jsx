...
export default function () {
	...
	const update = () => {
		if (!product.name.length || !String(product.price).length || !product.description.length || !product.ingredients.length || !product.cuisine.value) {
		...
		(async () => {
			...
			const [type, data] = await updateProduct({
				...
				images: product.images.length > 0 ? product.images : image[0].url,
			});
			if (type && data) {
	...
	useEffect(() => {
		(async () => {
			...
			if (type1 && type2) {
				const cuisine = data1.find((el) => el._id === data2.cuisine);
				setCuisines(data1.map((el) => ({ value: el._id, text: el.label })));
				setProduct({
					...data2,
					images: "", //data2.images[0].url,
					cuisine: { value: cuisine._id, text: cuisine.label },
				});
				setImage(data2.images);
			}
			setLoad(false);
		})();
	}, [_id]);
		...
		<UserSideBar>
			...
			>
				...
				{load ? (
					...
					<>
						<FileField label="Image" holder={image[0]?.url} value={product?.images} changed={(e) => setProduct({ ...product, images: e })} />
